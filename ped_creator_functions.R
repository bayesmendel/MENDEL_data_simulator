...
#' (not pathogenic) or `1` (pathogenic). Only provide a value when `proband = TRUE`.
...
#' the proband inherited from the founding mother. Must be `0` (not pathogenic)
...
create.person <- function(ped, motherid = NULL, fatherid = NULL, 
  ...
  if(founding.father){
    ...
    age.max <- ifelse(age.max > 95, 95, age.max)
    age.min <- partner.age - 10
    age.min <- ifelse(age.min < 16, 16, age.min)
  } else {
    sex <- sample(c("F","M"), 1)
  ...
  death <- ifelse(proband == 1, 0, rbinom(1, 1, prob = sum(death.pens)))
  if(death == 1 & age.min < age){
    age <- sample(age.min:age, size = 1, prob = death.pens)
  }
  ...
  if(proband == 1){
    ...
    a1 <- ifelse(!is.null(ff.allele), ff.allele, fm.allele)
    a2 <- rbinom(1, 1, prob = path.allele.prevalence)
    
    # partner that "married-in": two random alleles
  } else if(partner){
    a1 <- rbinom(1, 1, prob = path.allele.prevalence)
    a2 <- rbinom(1, 1, prob = path.allele.prevalence)
    
    # children of mothers and fathers in the pedigree, excluding the proband,
    # determined by Mendelian inheritance
  } else {
    a1 <- sample(c(ped$Allele1[ped$ID == motherid], 
                   ped$Allele2[ped$ID == motherid]), size = 1)
    a2 <- sample(c(ped$Allele1[ped$ID == fatherid], 
                   ped$Allele2[ped$ID == fatherid]), size = 1)
  }
  
  # switch allele positions if 1st position is pathogenic, and 2nd is not
  if(a1 == 1 & a2 == 0){
    allele1 <- a2
    allele2 <- a1
  } else {
...
create.test.fam <- function(proband.carriers,
  ...
  for(nc in 1:num.child){
    ...
    if(pb){ 
      pb.alleles <- c(pb.allele1, pb.allele2) 
    } else {
      pb.alleles <- NULL
    }
...
#'  pathogenic. All `NA`'s have been replaced with blanks.
...
create.test.fams <- function(num.fams, proband.carriers = NULL, 
  ...
  pb.statuses <- ifelse(is.null(proband.carriers), "1/1, 1/2, 2/2", 
                        ifelse(proband.carriers, "1/2, 2/2", "1/1"))
...
#' of which to retain. Any of `c("1/1", "1/2", "2/2")` which correspond to
#' no pathogenic alleles for `"1/1"`, heterozygous pathogenic alleles status of
#' `"1/2"`, and homozygous pathogenic allele status for `"2/2"`. Default is for 
#' all options with would apply no filtering.
